...
class MyResetPassword extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ...
            ->line(Lang::get('**Essa notificação é personalizada** Você está recebendo este e-mail porque recebemos uma solicitação de redefinição de senha'))
            ->action(Lang::get('Redefinir Senha'), $url)
            ->line(Lang::get('Este link de redefinição de senha expirará em :cont minutos.', ['count' => config('auth.passwords.' . config('auth.defaults.passords') . '.expire')]))
